...
- `video/`
  - 仅负责视频领域。
  - 包含视频后端 trait、各平台实现、显示器/屏幕采集。
- `recording/`
  - A/V 编排层。
  - 一次会话里接收可选的音频请求和可选的视频请求。
  - 支持音频-only、视频-only、音视频并行三种模式。
- `pipeline/`
...
`audio` 与 `video`（采集叶子域） -> `recording`（组合层）  
`pipeline`（处理域）与采集后端解耦，保持独立

CLI 层负责组装：
- `capture ...` 命令使用 `audio`
- `video ...` 命令使用 `video`
- `record run ...` 通过 `recording` 执行统一会话
- `pipeline ...` 命令使用 `pipeline`/`service`

## 为什么 `pipeline` 独立

`pipeline` 不是采集后端的一部分，而是采集后的处理层（ASR、翻译、事件流）。  
因此它应与 `audio`、`video` 并列但职责不同，保持独立最合理。

## 后续扩展路径

该布局天然支持：
- 各平台原生视频采集能力持续增强
- `recording` 增加 A/V 同步元数据
