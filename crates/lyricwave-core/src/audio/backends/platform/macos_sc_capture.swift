...
private func parseArguments() throws -> Arguments {
    ...
    while idx < argv.count {
        ...
        case "--out":
            idx += 1
            outPath = valueAt(argv, idx, for: "--out")
        case "--sample-rate":
            idx += 1
            let v = valueAt(argv, idx, for: "--sample-rate")
            sampleRate = UInt32(v) ?? sampleRate
        case "--channels":
            idx += 1
            let v = valueAt(argv, idx, for: "--channels")
            channels = UInt16(v) ?? channels
        case "--seconds":
            idx += 1
            let v = valueAt(argv, idx, for: "--seconds")
            seconds = UInt32(v)
        case "--pid":
            idx += 1
            let v = valueAt(argv, idx, for: "--pid")
            if let pid = Int32(v) {
                selectors.append(Selector(pid: pid, nameContains: nil))
            }
        case "--name":
            idx += 1
            let v = valueAt(argv, idx, for: "--name")
...
private func valueAt(_ argv: [String], _ idx: Int, for flag: String) -> String {
    if idx < argv.count {
        return argv[idx]
    }
    return ""
